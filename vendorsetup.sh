...
if ! [ -a $PATCH ]; then git clone https://github.com/nattolecats/vendor_oneplus_denniz-patches atches ; fi

# Include common patching functions
source vendor/oneplus/ivan-patches/functions.sh

# Discard existing patches
discardPatches

# Guard
if ! history | tail -n 1 | grep -q "ivan"; then return; fi
